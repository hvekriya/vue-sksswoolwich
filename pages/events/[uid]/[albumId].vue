  ...
  <div class="album-page-wrapper min-h-screen bg-black text-white">
    <UContainer class="py-12">
      ...
      <div class="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-12">
        <div>
          <UButton icon="i-heroicons-arrow-left" variant="ghost" color="white" label="Back to Event" class="-ml-4 mb-4"
            @click="navigateTo(`/events/${route.params.uid}`)" />
        ...
        <UButton v-if="flickrPhotosetWebUrl" :to="flickrPhotosetWebUrl" target="_blank" icon="i-simple-icons-flickr"
          label="Full Album" color="golden" variant="soft" class="rounded-xl px-6 shadow-lg shadow-golden-500/10" />
      </div>

      <!-- Loading State -->
      <div v-if="pending" class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4">
        <USkeleton v-for="i in 24" :key="i" class="aspect-square rounded-2xl bg-gray-900" />
      </div>
...
const flickrPhotosetWebUrl = computed(() => `https://www.flickr.com/photos/${config.public.flickrUserId}/sets/${albumId.value}/`)
