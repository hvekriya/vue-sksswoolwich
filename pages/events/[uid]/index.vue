    ...
    <template v-if="eventDetails">
      ...
      <section class="relative min-h-[50vh] flex items-end py-20 overflow-hidden">
        <div class="absolute inset-0 z-0">
          <img v-if="eventDetails.data.poster?.url" :src="eventDetails.data.poster.url"
        ...
        <UContainer class="relative z-10 w-full">
          ...
          <nav class="mb-8 flex items-center space-x-2 text-sm font-medium text-gray-500">
            <NuxtLink to="/events" class="hover:text-golden-600 transition-colors">Events</NuxtLink>
            <UIcon name="i-heroicons-chevron-right" class="w-4 h-4" />
            <span class="text-gray-900 dark:text-gray-100 line-clamp-1 truncate max-w-[200px]">
              {{ eventDetails.data.event_title[0]?.text }}
            </span>
          </nav>

          <div class="max-w-4xl">
            <h1 class="text-4xl lg:text-6xl font-serif font-bold text-gray-900 dark:text-white mb-6 leading-tight">
              {{ eventDetails.data.event_title[0]?.text }}
            </h1>

            <div class="flex flex-wrap gap-4 items-center text-gray-600 dark:text-gray-900">
              <div
              ...
              <div v-if="eventDetails.data.event_location"
      ...
      <UContainer class="py-16">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div class="lg:col-span-8">
            ...
            <div v-if="eventDetails.data.poster?.url || eventDetails.data.poster_2?.url" class="space-y-8 mb-16">
              <h3
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                <div v-if="eventDetails.data.poster?.url"
                  class="group relative overflow-hidden rounded-3xl shadow-2xl glass-effect border-golden-500/10">
                  <img :src="eventDetails.data.poster.url"
                    class="w-full h-auto transition-transform duration-700 group-hover:scale-110" />
                  <div
                    class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <UButton icon="i-heroicons-arrow-down-tray" color="white" variant="ghost"
                      :to="eventDetails.data.poster.url" target="_blank" />
                  </div>
                </div>
                <div v-if="eventDetails.data.poster_2?.url"
                  class="group relative overflow-hidden rounded-3xl shadow-2xl glass-effect border-golden-500/10">
                  <img :src="eventDetails.data.poster_2.url"
                    class="w-full h-auto transition-transform duration-700 group-hover:scale-110" />
                  <div
                    class="absolute inset-0 bg-black/20 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                    <UButton icon="i-heroicons-arrow-down-tray" color="white" variant="ghost"
                      :to="eventDetails.data.poster_2.url" target="_blank" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div v-if="photosetId" class="space-y-8">
              <div class="flex items-center justify-between">
                <h3
                  ...
                  Event Gallery</h3>
                <UButton v-if="flickrPhotosetWebUrl" :to="flickrPhotosetWebUrl" target="_blank"
                  icon="i-simple-icons-flickr" label="View on Flickr" variant="ghost" color="gray" />
              </div>
              ...
              <light-gallery v-else-if="album?.photo?.length"
                :settings="{ speed: 500, plugins: [lgThumbnail, lgZoom, lgFullscreen] }"
                class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4">
                <a v-for="(photo, index) in album.photo" :key="index" :href="photo.url_o"
                  ...
                  <div
                    ...
                    <UIcon name="i-heroicons-magnifying-glass-plus" class="w-6 h-6 text-golden-900" />
                  </div>
                </a>
              </light-gallery>
              ...
              <div v-else-if="!pending"
                ...
                <p class="text-gray-900 italic">No photos available yet. Check back soon!</p>
              </div>
            </div>
          </div>
          ...
          <div class="lg:col-span-4">
            <div class="sticky top-28 space-y-8">
              <!-- Share Card -->
              <UCard class="glass-effect overflow-hidden rounded-3xl border-golden-500/20">
                <h4
                  class="font-serif font-bold text-gray-900 bg-golden-500/5 px-6 py-4 border-b border-golden-500/10 flex items-center gap-2">
                  <UIcon name="i-heroicons-share" class="text-golden-600" />
                  Share Event
                </h4>
                <div class="p-6">
                  <p class="text-sm text-gray-500 mb-6">Invite your friends and family to join this celebration.</p>
                  <div class="grid grid-cols-2 gap-4">
                    <UButton block icon="i-simple-icons-facebook" color="blue" variant="soft" label="Facebook"
                      @click="shareOnFacebook" />
                    <UButton block icon="i-simple-icons-x" color="gray" variant="soft" label="Post"
                      @click="shareOnTwitter" />
                    <UButton block icon="i-heroicons-link" color="golden" variant="soft" label="Copy Link"
                      @click="copyLink" />
                  </div>
                </div>
              </UCard>
              ...
              <div
                ...
                <div class="relative z-10">
                  ...
                  <p class="text-white/80 text-sm mb-6 leading-relaxed">
                    If you have questions about this event or need assistance with attendance, feel free to reach out to
                    us.
                  </p>
                  <UButton to="/contact-us" color="white" variant="solid" label="Contact Us" block
...
const { data: eventDetails, error } = await useAsyncData(`event-${route.params.uid}`, () =>
...
const flickrPhotosetWebUrl = computed(() => photosetId.value ? `https://www.flickr.com/photos/${config.public.flickrUserId}/sets/${photosetId.value}/` : null)
...
const shareOnTwitter = () => {
  const url = encodeURIComponent(window.location.href)
  const text = encodeURIComponent(eventDetails.value?.data.event_title[0]?.text || 'Check out this event!')
...
useHead({
  ...
  meta: [
    { name: 'description', content: eventDetails.value?.data.event_description[0]?.text || '' }
  ]
})
</script>
